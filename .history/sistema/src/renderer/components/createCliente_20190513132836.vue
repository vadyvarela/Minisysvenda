            ...
            <v-card class="elevation-1">
              <v-card-text>
                <v-form ref="form" name="cadastar" autocomplete="off" v-model="valid" lazy-validation>
                 <v-text-field box append-icon="account_circle" required :rules="nameRules" name="cliente_nome" v-model="cliente.cliente_nome" :label="$t('message.cliente')" type="text"></v-text-field>
                    <v-text-field box append-icon="perm_identity" required :rules="nifRules" :counter="9" name="cliente_nif" v-model="cliente.cliente_nif" label="NIF" type="number"></v-text-field>
                    <v-text-field box append-icon="home" required :rules="enderecoRules" name="cliente_morada" v-model="cliente.cliente_morada" :label="$t('message.morada')" type="text"></v-text-field>
                    <v-text-field box append-icon="smartphone" required :rules="contatoRules" name="cliente_telefone" v-model="cliente.cliente_telefone" :label="$t('message.telefone')" type="number"></v-text-field>
                  <v-card-actions>
                    <v-spacer></v-spacer>
