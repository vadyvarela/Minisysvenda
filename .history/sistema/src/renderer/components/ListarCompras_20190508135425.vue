        ...
        <v-card-text>
          <v-container>
                ...
                <v-divider></v-divider>
                  ...
                  <div v-bind:key="index" v-for="(produto, index) in produtos.ListaCompras">
                      <h2 hidden> {{ banana = index }} </h2>
                      <div v-if="produtos.ListaCompras[index].Produto">
                        <h4 hidden>NOME: {{ produto.nome = produtos.ListaCompras[index].Produto.produto_nome }}</h4>
                        <h4 hidden>NOME: {{ produto.search = produtos.ListaCompras[index].Produto.produto_nome }}</h4>
                        <h4 hidden>PREÇO: {{ produto.preco = produtos.ListaCompras[index].Produto.produto_preco }}</h4>
                        <h4 hidden>ID PRODUTO: {{ produto.ProdutoId = produtos.ListaCompras[index].Produto.id }}</h4>
...
export default {
  ...
  computed: {
    ...
    totalQty() {
      return this.produtos.ListaCompras.reduce((total, produto) => {
