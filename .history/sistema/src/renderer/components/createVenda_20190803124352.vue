            ...
            <v-layout wrap>
                ...
                <div v-bind:key="index" v-for="(produto, index) in produtos">
                  ...
                  <div v-if="produto.Produto">
                    <h3 hidden>ID Pesquisa: {{ produto.idSearch = idSearch }}</h3>
                    <span hidden> {{ produto.adminPVenda = produtos[index].adminPVenda }} </span>
                    <h4 hidden>NOME: {{ produto.nome = produto.Produto.produto_nome }}</h4>
                    ...
                    <h2 hidden>PRECO VENDA 1: {{ produto.PVendaListOne = produto.Produto.PVendas[0] }} </h2>
                    <h2 hidden>SELECT PRECO VENDA: {{ produto.PVendaList = produto.Produto.PVendas }} </h2>
                    <h4 hidden>IVA: {{ produto.iva = produto.Produto.Iva.iva_valor }}</h4>
                    <h4 hidden>IVA: {{ produto.search = produto.Produto.produto_nome }}</h4>
                    <h4 hidden>ID PRODUTO: {{ produto.ProdutoId = produto.Produto.id }}</h4>
                  </div>
