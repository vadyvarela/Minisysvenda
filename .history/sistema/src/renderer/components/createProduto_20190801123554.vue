            ...
            <v-card class="elevation-0">
              <v-card-text>
                <v-form @submit.prevent="submit" ref="form" name="cadastar" autocomplete="off" lazy-validation>
                  ...
                  <v-flex xs12 sm2 md2>
                    <h3 hidden> {{ text = "Stock Atual: " }} </h3>
                    <v-text-field box color="blue" persistent-hint :hint="text + produto.Stock.quantidade" required v-model="produto.quantidade" label="Stock" type="number"></v-text-field>
                  </v-flex>
                  <v-flex xs12 sm3 md3>
