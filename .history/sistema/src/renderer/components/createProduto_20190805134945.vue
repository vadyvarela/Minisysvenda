              ...
              <v-card-text>
                <v-form @submit.prevent="submit" ref="form" name="cadastar" autocomplete="off" lazy-validation>
                  ...
                  <v-card-actions>
                    ...
                    <v-btn v-if="this.produto.id !== undefined" color="primary" :disabled="!formIsValid" @click="e1 = 2"> {{ $t('message.btnContinuar') }} </v-btn>
                    <v-btn v-if="this.produto.id !== undefined" color="success" @click="updateProd()"> Atualizar dados  </v-btn>
...
export default {
  ...
  methods: {
    ...
    async updateProd() {
      try {
        await PVendaServices.put(this.produto.PVendas);
  ...
  async mounted() {
    ...
    this.produto.FornecedoreId = this.FornecedoreId[0]
    this.IvaId = (await IvaService.index()).data;
    this.produto.IvaId = this.IvaId[0]
    this.CategoriaId = (await CategoriasService.index()).data;
    this.produto.CategoriaId = this.CategoriaId[0]
