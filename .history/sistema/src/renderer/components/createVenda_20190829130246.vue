                  ...
                  <v-layout class="layoutmeu">                    
                    ...
                    <v-flex flex xs12 sm2 md2>
                      <v-layout flex class="layoutmeu">
                        ...
                        <v-flex xs12 sm12 md12>
                          <v-text-field box class="header-text-field-input" id="styled-input" v-model.trim="produto.search" ref="search" autofocus v-on:keyup.enter="pesquisar(index)"></v-text-field>
                    ...
                    <v-flex xs12 sm2 md2>
                      ...
                      <div v-else>
                        <v-text-field box v-model.number="produto.quantidade" v-on:input="changeQuantidade(index)" @keyup.enter="putfocus(index)" ref="searchquantidade" label="Quantidade" type="number"></v-text-field>
                      </div>
                      <input v-model.number="produto.quantidade" v-on:input="changeQuantidade(index)" @keyup.enter="addNewProduto(index)" type="hidden"/>
                    </v-flex>
                    
                    <v-flex xs12 sm2 md2>
                      <v-text-field reverse ref="preco" box readonly v-if="user.nivel == 1 && produto.adminPVenda == ''" v-model.number="produto.preco_venda" @click="PVendachange(index)" name="" type="number"></v-text-field>
                    ...
                    <v-flex xs12 sm1 md1>
                      <v-text-field reverse box readonly ref="iva" :value="produto.iva" name="" type="number"></v-text-field>
                    </v-flex>
                    <v-flex xs12 sm2 md2>
                      <content>
...
export default {
  ...
  methods: {
      ...
      try {
        ...
        if (this.search !== '' ) {
          ...
          if (this.produtos[index].idProduto.length === 0) {
            ...
            this.$refs.search[index].focus();
          }else if (index > 0 && this.produtos[index - 1].idProduto.produto_barcode === this.search) {
             this.produtos[index - 1].quantidade = this.quantidade += 1
    ...
    async PrintVenda(index) {
        ...
        console.log("DADOS REC", recibo);

        const recibog = this.produtos
        console.log("DADOS REC TESTE", recibog);
