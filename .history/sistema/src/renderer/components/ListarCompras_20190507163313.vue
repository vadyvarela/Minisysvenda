              ...
              <v-stepper-content step="2">
                ...
                <v-divider></v-divider>
                  <v-layout wrap>
                    ...
                    <div v-bind:key="index" v-for="(pd, index) in produtos">
                      <h2 > index: {{ index }} </h2>
                      <h2> TESTE: {{ produtos[index] }} </h2>
                      <!--h2 hidden>{{ produto.CompraId = idCompra }}</h2-->
                      <div v-if="produtos[index].Produto">
                        <h4 >NOME: {{ pd.nome = produtos[index].Produto.produto_nome }}</h4>
                        <h4 >PREÇO: {{ pd.preco = produtos[index].Produto.produto_preco }}</h4>
                        <h4 >ID PRODUTO: {{ pd.ProdutoId = produtos[index].Produto.id }}</h4>
                        <span v-if="pd.preco">
                          <h4 >TOTAL LIQUIDO: {{ pd.totalLiquido = produto.quantidade * produto.preco_venda }}</h4>
                          <h4 >TOTAL: {{ pd.total = pd.quantidade * pd.preco }}*</h4>
                          <span > TOTAL IVA:  {{ pd.totalIva = pd.preco_venda * pd.iva / 100 }}</span>
                        </span>
                      </div>
                      <div v-else>
                        <h2 >Sorry Vady</h2>
                      ...
                      <v-layout class="layoutmeu">
                        <v-flex xs12 sm2 md2>
                          ...
                          <v-flex xs12 sm2 md2>
                            <v-text-field box readonly :value="pd.nome" type="text"></v-text-field>
                          </v-flex>
                          
                          <v-flex xs12 sm1 md1>
                            <v-text-field box reverse readonly :value="pd.preco" name="" type="number"></v-text-field>
                          </v-flex>
                          <!--v-flex xs12 sm2 md2>
                            <v-select
                              reverse
                              readonly
                              v-model="produto[index].idProduto.PVendas"
                              :items="produto[index].idProduto.PVendas"
                              chips
                              multiple
                              solo
                              box
                          ...
                          <v-flex right xs12 sm1 md1>
                            <v-text-field reverse box readonly :value="pd.iva" name="" type="number"></v-text-field>
                          </v-flex>
                          <v-flex xs12 sm2 md2>
                            <!--<v-text-field box readonly :value="produto.total" name="" type="number"></v-text-field>-->
                            <vue-numeric readonly style="border-bottom:1px solid #999; background:#f5f5f5; width:80%; padding:15px 3px 12px 4px; margin-right:-3px; font-size:1.4em; color: green; text-align:right" :value="pd.total"> </vue-numeric>                   
...
export default {
  ...
  methods: {
    ...
    editCompra: function(item) {
      ...
      this.pd =  this.lista.ListaCompras;
      console.log(this.pd)
      this.dialog = true;
