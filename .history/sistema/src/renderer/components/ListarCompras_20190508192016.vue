      ...
      <v-content >
        <v-layout justify-end row>
          ...
          <v-flex text-lg-right xs6>
            <v-btn router-link to="compras" text-lg-right class="primary"> NOVA COMPRA </v-btn>
          </v-flex>
        </v-layout>
          ...
          <v-container>
                ...
                <v-divider></v-divider>
                  ...
                  <div v-bind:key="index" v-for="(produto, index) in produtos">
                      ...
                      <h3 hidden>ID Pesquisa: {{ produto.idSearchNew = idCompra }}</h3>
                      <div hidden v-if="produto.Produto && produto.Produto.Fornecedore">
                        <h3>ID Pesquisa: {{ produto.idSearch = '' }}</h3>
                        <h4 hidden>ID PRODUTO: {{ produto.ProdutoId = produto.Produto.id }}</h4>
                      </div>
                      <div hidden v-else-if="produto.Produto && !produto.Produto.Fornecedore">
                        <h3>ID: {{ produto.idSearch = idCompra }}</h3>
                        <h4 hidden>ID PRODUTO: {{ produto.ProdutoId = produto.Produto.id }}</h4>
                      </div>
                      ...
                      <v-navigation-drawer width="700" temporary fixed :clipped="clipped" v-model="left" enable-resize-watcher app right >
                        ...
                        <v-flex xs12 sm4 md4>
                          ...
                          <v-text-field box v-model.trim="produto.search" ref="search" autofocus v-on:keyup.enter="pesquisarProdR(index)" type="text"></v-text-field>
          ...
          <v-container>
            <v-layout style="margin-top:-22px;" wrap>
              ...
              <v-flex class="text-xs-right" xs12 sm2 md2>
                <v-btn color="white" flat v-shortkey="['ctrl','enter']" @shortkey="updateCompraProd()" @click="updateCompraProd()">
                  <span style="font-size:1.5em;"> Atualizar Dados compra </span>
                  <v-icon>send</v-icon>
                </v-btn>
              </v-flex>
            </v-layout>
  ...
  methods: {
    ...
    removeNewProdutoUpdate(index) {
      ...
      if(this.produtos[index].idSearch == null || this.produtos[index].idSearch == '' ){
        ...
        stockServices.putstockcompra(this.produtos[index])
        listaCompraServices.delete(this.produtos[index])
        this.$toast.error({
          title: "Aviso",
    ...
    async updateCompraProd() {
      listaCompraServices.postnewprod(this.produtos)
      ...
      this.$toast.success({
        title: "Sucesso",
        message: "Compra atualizada com sucesso no sistema"
      })
    },
    async backToCat () {
      this.left = true
    ...
    async pesquisarbyCat(index, nome) {
      ...
      try {
        ...
        if (this.search !== '') {
          this.produtos[index].Produto = (await filterServices.bynamevenda(this.search)).data
          console.log(this.produtos[index].Produto)
          Object.keys(this.produtos[index].Produto).forEach(key => {
            this.produtos[index].Produto = this.produtos[index].Produto[key];
          });
          if (this.produtos[index].Produto.length === 0) {
            this.snackbar = true
    ...
    changeQuantidade (index) {
      ...
      if(this.produtos[index].idSearch == null || this.produtos[index].idSearch == '' ){
        ...
        stockServices.putstockaddremovecompra(this.produtos[index])
        this.$toast.success({
          title: "Aviso",
    ...
    async pesquisarProdR(index, nome) {
      try {
        ...
        if (this.search !== '') {
          ...
          console.log("PROD-----", this.produtos[index].Produto)
          Object.keys(this.produtos[index].Produto).forEach(key => {
            this.produtos[index].Produto = this.produtos[index].Produto[key];
          });
          if (this.produtos[index].Produto.length === 0) {
    ...
    async pesquisarProd(banana) {
      ...
      try {
        ...
        if (this.search !== '') {
          ...
          console.log("PROD " + this.produtos[banana].Produto)
