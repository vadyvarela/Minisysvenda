              ...
              <v-stepper-content step="2">
                ...
                <v-divider></v-divider>
                  <v-layout wrap>
                    ...
                    <div v-bind:key="index" v-for="(produtos, index) in produto">
                      <h2 > index: {{ produto[index] }} </h2>
                      <!--h2 hidden>{{ produto.CompraId = idCompra }}</h2-->
                      <div v-if="produto[index].idProduto && produto[index].idProduto.Iva">
                        <h4 >NOME: {{ produto.nome = produto[index].idProduto.produto_nome }}</h4>
                        <h4 >IVA: {{ produto.iva = produto[index].idProduto.Iva.iva_valor }}</h4>
                        <h4 >PREÇO: {{ produto.preco = produto[index].idProduto.produto_preco }}</h4>
                        <h4 >ID PRODUTO: {{ produto.ProdutoId = produto[index].idProduto.id }}</h4>
