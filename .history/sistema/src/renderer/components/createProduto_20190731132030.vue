            ...
            <v-card class="elevation-0">
              <v-card-text>
                <v-form @submit.prevent="submit" ref="form" name="cadastar" autocomplete="off" lazy-validation>
                  ...
                  <v-layout wrap>
                    <div hidden v-if="this.produto.idProduto && this.produto.idProduto.Iva">
                      <!-- <h4>PROD COD: {{ produto.produto_code = this.produto.idProduto.produto_code }}</h4> -->
                      <h4>PROD BARCODE: {{ produto.produto_barcode = this.produto.idProduto.produto_barcode }}</h4>
                      <h4>NOME PROD: {{ produto.produto_nome = this.produto.idProduto.produto_nome }}</h4>
                      <h4>NOME PROD REC: {{ produto.produto_nome_rec = this.produto.idProduto.produto_nome_rec }}</h4>
                      <h4>STOCK: {{ produto.quantidade = this.produto.idProduto.Stock.quantidade }}</h4>
                      <h4>PRECO CUSTO: {{ produto.produto_preco = this.produto.idProduto.produto_preco }}</h4>
                      <h4>IVA: {{ produto.IvaId = this.produto.idProduto.Iva }}</h4>
                      <h4>FORNECEOR: {{ produto.FornecedoreId = this.produto.idProduto.Fornecedore }}</h4>
                      <h4>CATEGORIA: {{ produto.CategoriaId = this.produto.idProduto.Categoria }}</h4>
          ...
          <v-flex xs12 sm12 md12>
              <v-btn small fab class="primary" v-shortkey="['ctrl','n']" @shortkey="addNewPrice" @click="addNewPrice"><v-icon>add</v-icon></v-btn>
                <v-form ref="form" name="cadastar" autocomplete="off" lazy-validation>
                  ...
                  <v-container grid-list-md>
                    VADY {{ this.produto.idProduto.PVendas }}
                  <div v-bind:key="index" v-for="(preco, index) in this.produto.idProduto.PVendas">
                        <h5>Preco de venda #{{ index }}</h5>
                        <h2 > {{ preco.ProdutoId = idProduto }}</h2>
                        <span  v-if="produto.IvaId === 2"> {{ totalIva = produto.produto_preco * (1 + 0 / 100 ) }}</span>
                        <span  v-else> {{ totalIva = produto.produto_preco * (1 + 15 / 100 ) }}</span>
                        <span > {{ total = (( preco.pvenda_preco - produto.produto_preco) / preco.pvenda_preco) * 100 }}</span>
                          <v-layout>
                            <v-flex xs12 sm4 md4>
                  ...
                  <v-card-actions>
                    <v-spacer></v-spacer>
                    <div v-if="this.produto.idProduto " class="text-xs-center">
                      <v-btn color="success" large :disabled="!PVendaIsValid" @click="updatePrecos(), updateProd()"> Atualizar  </v-btn>
                    </div>
...
export default {
  ...
  methods: {
    ...
    async pesquisar (){
      ...
      try {
        ...
        if (this.search !== '') {
          ...
          if (this.produto.idProduto.length === 0) {
            this.snackbar = true;
            this.produto = {
              produto_code: null,
              produto_nome: null,
    ...
    async updateProd () {
      console.log("Teste update produto");
    },
    async cadProd() {
      try {
        this.meuloading = true
