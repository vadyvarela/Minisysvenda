<template>
    <panel :title="$t('message.novaVenda')">
      ...
      <v-snackbar
        ...
        :vertical="mode === 'vertical'">
        {{ textsucesso }}
        <v-btn dark flat @click="snackbarsucesso = false"> Fechar </v-btn>
      </v-snackbar>
        ...
        <v-card-text>
            ...
            <v-card align-center justify-center>
              ...
              <v-card-text>
                <v-container grid-list-md>
                  <v-layout wrap>
                    <v-flex xs12>
                      <h4 class="primary--text text-md-center" style="font-size:1em;"> {{ $t('message.searchcliente') }} </h4>
                      <p class="red--text" style="text-align:center; font-size:1em;">
                        <v-autocomplete
                          box
                          :items="listaclientes"
                          color="white"
              ...
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn center small  color="primary darken-1" @click.native="dialogPesquisaCliente = false; pesquisarCliente()"> {{ $t('message.ok') }} </v-btn>
              </v-card-actions>
            </v-card>
          </v-dialog>
          <v-dialog v-model="dialogNewCliente" persistent max-width="550px" @keydown.esc="dialogNewCliente = false">
              <v-card align-center justify-center>
                ...
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn center small  color="primary darken-1" @click.native="createCliente()"> {{ $t('message.salvar') }} </v-btn>
                </v-card-actions>
              </v-card>
          </v-dialog>
          ...
          <v-flex xs6 sm3 md3>
            <v-layout style="margin-top:-10px">
              <v-flex sm3 md3>
                <v-text-field height="10" small  outline label="N. Venda" @keyup.enter="searchIdProd()" v-model.number="idSearch" type="text"></v-text-field> 
              </v-flex>
              <v-flex sm7 md7>
              ...
              <v-flex sm1 md1>
                ...
                <v-btn small  left router-link to="listavendas" dark class="primary"> {{ $t('message.listavendas') }} </v-btn>
                <span v-if="idSearch !== ''" sm2 md2>
                  <v-btn small class="success" @click.stop="reset"> {{ $t('message.novaVenda') }} <v-icon>loop</v-icon> </v-btn>
                </span>
              </v-flex>
            </v-layout>
        ...
        <v-card-text>
            ...
            <v-layout wrap>
                ...
                <div v-bind:key="index" v-for="(produto, index) in produtos">
                  ...
                  <!-- <h3>ID Pesquisa: {{ produtos[index].idProduto }}</h3> -->
                  <div hidden v-if="produto.Produto">
                    <h3 >ID Pesquisa: {{ produto.idSearch = idSearch }}</h3>
                    <span > {{ produto.adminPVenda = produtos[index].adminPVenda }} </span>
                    ...
                    <h2  v-if="user.nivel == 1 && produto.adminPVenda  == ''">PRECO VENDA ADMIn: {{ produto.preco_venda = produto.Produto.PVendas[0].pvenda_preco }} </h2>
                  ...
                  <div hidden v-if="produtos[index].idProduto && produtos[index].idProduto.Iva">
                    <span > {{ produto.adminPVenda = produtos[index].adminPVenda }} </span>
                    <h2  v-if="user.nivel == 2 && userconfig != 1">PRECO VENDA: {{ produto.preco_venda = produtos[index].idProduto.PVendas[0].pvenda_preco }} </h2>
                    <h2  v-if="user.nivel == 1 && produto.adminPVenda == '' ">PRECO VENDA ADMIn: {{ produto.preco_venda = produtos[index].idProduto.PVendas[0].pvenda_preco }} </h2>
                    <h2 >PRECO VENDA 1: {{ produto.PVendaListOne = produtos[index].idProduto.PVendas[0] }} </h2>
                    <h2  >SELECT PRECO VENDA: {{ produto.PVendaList = produtos[index].idProduto.PVendas }} </h2>
                    <h4 >NOME: {{ produto.nome = produtos[index].idProduto.produto_nome }}</h4>
                    <h4 >IVA: {{ produto.iva = produtos[index].idProduto.Iva.iva_valor }}</h4>
                    <h4 >ID PRODUTO: {{ produto.ProdutoId = produtos[index].idProduto.id }}</h4>
                  </div>
                  <div v-else>
                  ...
                  <v-layout class="layoutmeu">                    
                    ...
                    <v-flex flex xs12 sm2 md2>
                      <v-layout flex class="layoutmeu">
                        <v-btn style="display:none;" v-shortkey="['f2']" @shortkey="searchProd" @click.stop="searchProd"></v-btn>
                        <v-btn style="display:none;" v-shortkey="['f3']" @shortkey="searchProdCat" @click.stop="searchProdCat"></v-btn>
                        <v-flex xs12 sm12 md12>
                          <v-text-field class="styled-input" id="styled-input" v-model.trim="produto.search" ref="search" autofocus v-on:keyup.enter="pesquisar(index)"></v-text-field>
                    ...
                    <v-flex xs12 sm3 md3>
                      <v-text-field readonly :value="produto.nome" type="text"></v-text-field>
                    </v-flex>
                    <v-flex xs12 sm2 md2>
                      <div v-if="idSearch == '' && typeof produtos[index].idProduto.produto_nome === 'undefined'">
                        <v-text-field type="hidden"></v-text-field>
                      </div>
                      <div v-else>
                        <v-text-field v-model.number="produto.quantidade" v-on:input="changeQuantidade(index)" @keyup.enter="putfocus(index)" ref="searchquantidade" label="Quantidade" type="number"></v-text-field>
                      </div>
                      <input v-model.number="produto.quantidade" v-on:input="changeQuantidade(index)" @keyup.enter="addNewProduto(index)" type="hidden"/>
                    </v-flex>
                    
                    <v-flex xs12 sm2 md2>
                      <v-text-field reverse ref="preco" readonly v-if="user.nivel == 1 && produto.adminPVenda == ''" v-model.number="produto.preco_venda" @click="PVendachange(index)" name="" type="number"></v-text-field>
                      ...
                      <v-select
                       ...
                       color="blue"
                        ...
                        item-value="pvenda_preco"></v-select>
                      ...
                      <v-text-field reverse readonly v-if="userconfig != 1 && user.nivel == 2" v-model.number="produto.preco_venda" @click="changePVenda" name="" type="number"></v-text-field>
                    ...
                    <v-flex xs12 sm1 md1>
                      <v-text-field reverse readonly ref="iva" :value="produto.iva" name="" type="number"></v-text-field>
                    </v-flex>
                    <v-flex xs12 sm2 md2>
                      <content>
                      ...
                      <v-btn style="display:none;" v-shortkey="['del']" @shortkey="removeNewProdutoUpdate(index)" @click.native="removeNewProdutoUpdate(index)"></v-btn>
                      </content>
                    </v-flex>
                    <v-spacer></v-spacer>
    ...
    <v-dialog @keydown.enter="pesquisarn(banana)" v-model="dialogPesquisa" persistent max-width="550px" @keydown.esc="dialogPesquisa = false">
      <v-card align-center justify-center>
        <v-card-title>
          <v-spacer></v-spacer>
    ...
    <v-dialog @keydown.space="createVendaProdnoPrinter(banana)" @keydown.enter="createVendaProd(banana)" v-model="dialog" persistent max-width="550px" @keydown.esc="dialog = false">
...
export default {
  data() {
    return {
      ...
      colorsucesso: 'success',
      textsucesso: 'Venda efetuado com sucesso no sistema!',
  ...
  methods: {
      ...
      try {
        ...
        if (this.search !== '') {
          ...
          if (this.produtos[banana].idProduto.length === 0) {
            ...
            this.$refs.search[banana].reset();
            this.$refs.search[banana].focus();
          }else if (banana > 0 && this.produtos[banana - 1].idProduto.produto_nome === this.search ) {
             this.produtos[banana - 1].quantidade = this.quantidade += 1

             this.produtos.splice(banana, 1)
             this.produtos.push({
              adminPVenda: '',
              idSearchNew: '',
              total: '',
            ...
            this.$refs.search[banana].focus();
          }else if (banana > 0 && this.produtos[banana - 1].idProduto.produto_code === this.search) {
             this.produtos[banana - 1].quantidade = this.quantidade += 1
    ...
    async pesquisar(index) {
      ...
      try {
        ...
        if (this.search !== '' ) {
          ...
          /*if (this.produtos[index].idProduto.length === 0 && !isNaN(this.search)) {
              this.search = this.produtos[index].search
              this.meuloading = true
              this.meuloading = false;
              this.produtos[index].idProduto = this.produtos[index].idProduto = [{
              "produto_code": "Sem Cod", 
              "produto_nome": "Sem Cod", 
              "produto_nome_rec": "Sem Cod",
              "Iva": {
                "iva_valor": 15, 
              }, "Stock": {
                "ProdutoId": 0 
              }, "PVendas": [{
                "pvenda_preco": this.search,
                "ProdutoId": 0 }] 
              }]
              this.meuloading = false
              Object.keys(this.produtos[index].idProduto).forEach(key => {
              ...
              this.addNewProduto(index)
              console.log("DDD:::: ", this.produtos[index].idProduto);
          }
          else */if (this.produtos[index].idProduto.length === 0) {
            this.snackbar = true
            this.$refs.search[index].reset();
            this.$refs.search[index].focus();
          }
          else if (index > 0 && this.produtos[index - 1].idProduto.produto_nome === this.search ) {
             this.produtos[index - 1].quantidade = this.quantidade += 1
            ...
            this.$refs.search[index].focus();
          }else if (index > 0 && this.produtos[index - 1].idProduto.produto_code === this.search) {
             this.produtos[index - 1].quantidade = this.quantidade += 1
    ...
    async createVendaProdnoPrinter(banana) {
      if (this.pagamento.troco < 0) {
        this.$toast.error({
          title: "Erro",
          message: "O Valor inserido não pode ser menor que o Total a Pagar!"
        })
      } else {
        try{
          ...
          console.log("VENDA SEM USO DE RECIBO")
    ...
    async createVendaProd(banana) {
      if (this.pagamento.troco < 0) {
        ...
        try{
          ...
          console.log("VENDA COM RECIBO DE VENDA")
          this.meuloading = true
          await listaVendaServices.post(this.produtos)
          await stockServices.putvendas(this.produtos)
          
          // if(this.pagamento.valorentregadocheque == "") {
          //   this.pagamento.valorentregadocheque = 0
          // }
          // if(this.pagamento.valorentregado == "") {
          //   this.pagamento.valorentregado = 0
          // }
          // if(this.pagamento.valorentregadovint4 == "") {
          //   this.pagamento.valorentregadovint4 = 0
          // }

          await VendaServices.putidpagamento(this.pagamento)
