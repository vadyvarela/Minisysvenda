    ...
    <panel title="LISTA DE COMPRAS">
      ...
      <v-container>
            ...
            >
                ...
                <td>{{ props.item.Produto.produto_nome }}</td>
                <td class="text-xs-center ">{{ props.item.Produto.produto_preco }}</td>
                <td class="text-xs-left">{{ props.item.Compra.Fornecedore.fornecedor_nome }}</td>
                <td class="text-xs-left">{{ props.item.quantidade }} </td>
                <td class="text-xs-left"> <span class="boxquantidadegreen"> {{ props.item.quantidade * props.item.Produto.produto_preco }} CVE </span> </td>
                </tr>
                </template>
                <template slot="no-data">

                  <p v-if="total == 0" class="noDataLoading"> NEHMUM RESULTADO ENCONTRADO </p>
                  <p v-else class="noDataLoading"> {{ noDataLoading }} </p>
                </template>
