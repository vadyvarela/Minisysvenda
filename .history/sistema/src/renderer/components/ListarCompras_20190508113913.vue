                ...
                <v-divider></v-divider>
                  <v-layout wrap>
                    ...
                    <div v-bind:key="index" v-for="(pd, index) in produto">
                      <h2 hidden> index: {{ banana = index }} </h2>
                      <!-- <h2> TESTE: {{ produto[index] }} </h2> -->
                      <!--h2 hidden>{{ produto.CompraId = idCompra }}</h2-->
                      <div v-if="produto[index].Produto && produto[index].Produto.Iva">
                        <h4 hidden>NOME: {{ produto.nome = produto[index].Produto.produto_nome }}</h4>
                        <h4 hidden>PESQUISA: {{ produto.search = produto[index].Produto.produto_nome }}</h4>
                        <h4 hidden>QUANTIDADE: {{ produto.quantidade = produto[index].quantidade }}</h4>
                        <h4 hidden>PREÇO: {{ produto.preco = produto[index].Produto.produto_preco }}</h4>
                        <h4 hidden>ID PRODUTO: {{ produto.ProdutoId = produto[index].Produto.id }}</h4>
                      ...
                      <v-layout class="layoutmeu">
                        ...
                        <v-flex xs12 sm2 md2>
                          <v-text-field box v-model.number="produto.quantidade" @keyup.enter="addNewProduto(banana)" ref="searchquantidade" label="Quantidade" type="number"></v-text-field>
                          <input v-model.trim="produto.quantidade" @keyup.enter="addNewProduto(banana)" type="hidden" autofocus/>
                        </v-flex>
                        <v-flex xs12 sm1 md1>
...
export default {
  ...
  computed: {
    ...
    ListaProdIsValid () {
      return (
        this.produto.CompraId &&
        this.produto.ProdutoId
      )
    },
    totalQty() {
      return this.produto.reduce((total, produto) => {
