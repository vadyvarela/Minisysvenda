...
export default {
  ...
  methods: {
    ...
    async sendFile() {
      ...
      formData.append('file', this.fileBuffer, { filename: 'my-file.ext' })
      formData.append('categoria_nome', 'VADY')
      formData.append('categoria_desc', 'VARELA')
      formData.append('my_buffer', new Buffer(10));
      //formData.append('my_file', fs.createReadStream('/foo/bar.jpg'));

      try {
        this.meuloading = true
        await CategoriasService.post(formData).then((result)=>{
