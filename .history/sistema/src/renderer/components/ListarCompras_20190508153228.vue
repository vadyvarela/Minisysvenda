                  ...
                  <div v-bind:key="index" v-for="(produto, index) in produtos">
                      ...
                      <v-navigation-drawer width="700" temporary fixed :clipped="clipped" v-model="left" enable-resize-watcher app right >
                        ...
                        <v-flex xs12 sm4 md4>
                          ...
                          <v-text-field box v-model.trim="produto.search" ref="search" autofocus v-on:keyup.enter="pesquisarProd(banana)" type="text"></v-text-field>
...
export default {
  ...
  methods: {
    ...
    async pesquisarProd(index) {
      try {
        this.search = this.podpesquisa
        this.produtos[index].search = this.podpesquisa
        this.produtos[index].Produto = (await filterServices.bynamecompra(this.search)).data
        ...
        console.log('PRODUTO', this.produtos[index].Produto)
      } catch (err) {
        console.log(err);
      }
    },
