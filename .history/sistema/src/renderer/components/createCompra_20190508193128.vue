          ...
          <v-container grid-list-md>
            <v-layout wrap>
              <v-flex xs12>
                ...
                <p class="red--text" style="text-align:center; font-size:2em;">
                  <v-autocomplete box
                    :items="listaprodutos" color="white"
                    v-model="podpesquisa"
                    item-text="produto_nome"
                    item-value="produto_nome"
                    label="Nome produto"
  ...
  methods: {
    ...
    async pesquisar(banana) {
      try {
        ...
        this.produtos[banana].idProduto = (await filterServices.bynamecompra(this.search)).data
        ...
        console.log('PRODUTO', this.produtos[banana].idProduto)
      } catch (err) {
        console.log(err);
      }
    },
