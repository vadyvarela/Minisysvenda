...
module.exports = {
  ...
  async bynamevenda (req, res) {
    try {
      ...
      const result = await Produtos.findAll({
        include: [
          { model: Fornecedores, required: true, separate: true },
          { model: Categorias, required: true, separate: true },
          { model: Iva, required: true, separate: true },
          { model: Stock, required: true, separate: true },
          { model: PVenda, required: true, separate: true, order: [ ['id', 'ASC'] ] }
        ],
        where: {
          $or: [
            'produto_code', 'produto_barcode', 'produto_nome'
