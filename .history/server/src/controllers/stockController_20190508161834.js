const { Stock, ListaVenda, ListaCompra, Sequelize } = require('../models')
...
module.exports = {
  ...
  // Adiciona ou remove stock de acordo com quantidade seleciona na edicao de venda
  async putstockaddremovecompra (req, res) {
    const dados = req.body
    let value = dados
    const listacompras = await ListaCompra.findOne({
      where: {
        ProdutoId: value.ProdutoId,
        CompraId: value.idSearch
      }
    })
    console.log('QUANTIDADE ATUAL --- ', listacompras.quantidade)
    console.log('QUANTIDADE NOVO --- ', value.quantidade)
    if (listacompras.quantidade > value.quantidade) {
      console.log('ADICIONAR')
      try {
        const qtd = listacompras.quantidade - value.quantidade
        console.log('RES ----- ', qtd)
        Stock.update({ quantidade: Sequelize.literal('quantidade - ' + qtd) }, { where: { ProdutoId: value.ProdutoId } })
        ListaCompra.update({ quantidade: value.quantidade }, { where: { ProdutoId: value.ProdutoId, CompraId: value.idSearch } })
      } catch (err) {
        res.status(500).send({
          error: 'Um erro ocoreu ao tentar update STOCK'
        })
      ...
      console.log('REMOVER')
      try {
        const qtd = value.quantidade - listacompras.quantidade
        console.log('RES ----- ', qtd)
        Stock.update({ quantidade: Sequelize.literal('quantidade + ' + qtd) }, { where: { ProdutoId: value.ProdutoId } })
        ListaCompra.update({ quantidade: value.quantidade }, { where: { ProdutoId: value.ProdutoId, CompraId: value.idSearch } })
      } catch (err) {
  ...
  // Remove da stock quando um produto e removido da lista de compra
  putstockcompra (req, res) {
    const dados = req.body
    let value = dados
    try {
      Stock.update({ quantidade: Sequelize.literal('quantidade - ' + value.quantidade) }, { where: { ProdutoId: value.ProdutoId } })
