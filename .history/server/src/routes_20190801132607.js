...
const { Categorias, Produtos, Stock, Sequelize } = require('./models')
        ...
        if (req.file === undefined) {
          try {
            const stock = Produtos.update({
              ...
              Stock.update({ quantidade: Sequelize.literal('quantidade + ' + req.body.quantidade) }, { where: { ProdutoId: req.body.id } })
            })
            res.send(stock)
            // res.send(produtos)
          } catch (err) {
            res.status(500).send({
              error: 'Um erro ocoreu ao tentar update produto'
            })
          }
        } else {
          try {
